...
public enum DropState {
  ...
  private func backgroundColor() -> UIColor? {
    ...
    case Error: return UIColor(white: 255.0 / 255.0, alpha: 1.0)
...
public final class Drop: UIView {
  ...
  override init(frame: CGRect) {
    ...
    NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(Drop.deviceOrientationDidChange(_:)),
  ...
  private func scheduleUpTimer(after: Double, interval: Double) {
    stopUpTimer()
    upTimer = NSTimer.scheduledTimerWithTimeInterval(after, target: self, selector: #selector(Drop.upFromTimer(_:)),
